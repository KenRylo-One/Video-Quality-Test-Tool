...
pub fn write_run(
    ...
    outcome: &RunOutcome,
    fonts: &[&[u8]],
) -> vqtt_core::Result<Exported> {
    let folder = parent.join(format!("vqtt-{}", safe_name(&outcome.run_id)));
    std::fs::create_dir_all(&folder)
        .map_err(|error| vqtt_core::CoreError::parse("export", error.to_string()))?;
...
mod tests {
    ...
    use vqtt_core::media::{ColorRange, MediaInfo};
    ...
    fn outcome_for(session: &Session) -> RunOutcome {
        ...
        results.insert(
            ...
            pool(&values, vqtt_core::metric::HarmonicMean::Allowed).unwrap(),
        );
    ...
    fn export_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join("vqtt-export-test").join(name);
    ...
    fn a_folder_is_named_for_the_run_and_holds_everything_together() {
        ...
        assert_eq!(
            ...
            "vqtt-2026-09-05T01-02-03Z-abcd"
        );
