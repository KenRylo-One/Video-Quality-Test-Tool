...
use vqtt_backends::parse::stats_file::parse_stats_file;
use vqtt_core::backend::{BufferSink, Invocation, ProcessRunner, Progress};
use vqtt_core::capability::LaneKind;
use vqtt_core::metric::MetricId;
use vqtt_core::pooling::{Pooled, pool};
...
impl ProcessRunner for RealProcessRunner {
    fn run(
    ...
    ) -> vqtt_core::Result<vqtt_core::backend::ExitReport> {
        ...
        let mut child = command
            .spawn()
            .map_err(|error| vqtt_core::CoreError::parse("process", error.to_string()))?;
        ...
        Ok(vqtt_core::backend::ExitReport {
...
fn read_pooled(
    artifact: &vqtt_core::backend::LogArtifact,
    metric: MetricId,
...
mod tests {
    ...
    use vqtt_core::CoreError;
    use vqtt_core::backend::{ExitReport, LogArtifact, LogFormat};
    use vqtt_core::capability::BinaryId;
    ...
    fn write_fixture(name: &str, content: &str) -> PathBuf {
        let dir = std::env::temp_dir().join("vqtt-supervisor-test");
    ...
    fn quiet_long_program() -> (&'static str, Vec<&'static str>) {
        if cfg!(windows) {
            ("waitfor", vec!["/t", "20", "VqttCancelTest"])
        } else {
            ("sleep", vec!["20"])
        }
    }
    ...
    fn the_reason_a_back_end_gives_survives_repeats_and_a_trailing_source_line() {
        ...
        assert!(
            message.contains("Reduce or Set numStream argument"),
            "{message}"
        );
        assert_eq!(
    ...
    fn every_invocation_reports_that_it_started_and_that_it_stopped() {
        ...
        assert_eq!(
            order,
            vec![("started", MetricId::PsnrY), ("done", MetricId::PsnrY),]
        );
    }
