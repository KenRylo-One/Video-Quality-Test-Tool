...
use vqtt_backends::vmaf_model::read_model_folder;
...
mod tests {
    ...
    fn an_override_that_holds_no_model_folder_finds_nothing() {
        let temp = std::env::temp_dir().join("vqtt-vmaf-model-search-test-empty");
    ...
    fn an_override_that_holds_the_real_subfolder_is_found() {
        let temp = std::env::temp_dir().join("vqtt-vmaf-model-search-test-real");
    ...
    fn a_high_frame_rate_reference_looks_in_the_hfr_subfolder() {
        let temp = std::env::temp_dir().join("vqtt-vmaf-model-search-test-hfr");
