...
use vqtt_backends::ffprobe::FfprobeProbe;
use vqtt_backends::{discovery, hash};
use vqtt_core::capability::{BinaryId, Inventory};
use vqtt_core::estimate::{RunEstimate, estimate};
use vqtt_core::media::{ColorRange, FrameSample, LumaExtremes, MediaInfo, Rational};
use vqtt_core::metric::{Availability, MetricId, availability};
use vqtt_core::preset::PRESETS;
use vqtt_core::probe::MediaProbe;
use vqtt_core::set::{ComparisonSet, FileId};
...
mod tests {
    ...
    fn session_with(filters: &[&str]) -> Session {
        ...
        if !filters.is_empty() {
            ...
            let libvmaf = vqtt_backends::capabilities::libvmaf_features(&names);
