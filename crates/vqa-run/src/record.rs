...
use vqa_core::corrections::{Correction, Note};
use vqa_core::media::MediaInfo;
...
const GIT_SHA: Option<&str> = option_env!("VQA_GIT_SHA");
...
impl InvocationRecord {
    pub fn of(seq: u32, invocation: &vqa_core::backend::Invocation) -> Self {
        Self {
            seq,
...
fn file_record(file: &vqa_core::set::MediaFile, role: &'static str) -> FileRecord {
    FileRecord {
        role,
        label: file.label.clone(),
        path: file.info.path.clone(),
        bytes: file.info.bytes,
        fingerprint: vqa_backends::hash::fingerprint_file(&file.info.path)
            .ok()
...
fn plan_record(
    ...
    PlanRecord {
        ...
        frame_range: outcome.frame_range.map(|(first, last)| FrameRange {
            first,
            last,
        }),
        ...
        for def in vqa_core::metric::REGISTRY.iter() {
            ...
            records.push(ResultRecord {
                ...
                harmonic_mean_blocked: match def.harmonic_mean {
                    vqa_core::metric::HarmonicMean::Blocked(reason) => Some(reason),
                    _ if pooled.harmonic_mean.is_none() => {
                        Some("The values reach zero or below, where the harmonic mean has no meaning.")
                    }
                    _ => None,
                },
...
mod tests {
    ...
    fn a_label_that_a_file_system_would_refuse_becomes_a_usable_name() {
        assert_eq!(frame_csv_name("YT 1080p/50Mbps"), "frames-YT_1080p_50Mbps.csv");
