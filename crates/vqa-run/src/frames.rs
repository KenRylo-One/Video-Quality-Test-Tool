...
use vqa_backends::ffmpeg::plan_frame_extract;
use vqa_core::backend::{Invocation, MeasureJob};
use vqa_core::{CoreError, Result};
...
mod tests {
    ...
    fn a_binary_that_writes_no_image_gives_an_error_and_never_a_silent_pass() {
        let dir = std::env::temp_dir().join("vqa-frames-missing");
    ...
    fn the_commands_are_reported_so_a_reader_can_repeat_the_extraction() {
        let dir = std::env::temp_dir().join("vqa-frames-commands");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn a_path_with_a_space_in_it_is_quoted_so_the_command_can_be_pasted_back() {
        assert_eq!(quote("C:/Program Files/a.exe"), "\"C:/Program Files/a.exe\"");
