...
pub fn write_command_log(output_path: &Path, outcome: &RunOutcome) -> vqa_core::Result<()> {
    let file = std::fs::File::create(output_path).map_err(io_error)?;
    let mut writer = BufWriter::new(file);

    writeln!(writer, "# vqa command log, run {}", outcome.run_id).map_err(io_error)?;
...
fn io_error(error: std::io::Error) -> vqa_core::CoreError {
    vqa_core::CoreError::parse("csv", error.to_string())
}
...
mod tests {
    ...
    fn a_blocked_harmonic_mean_writes_an_empty_cell_and_the_label_holds_its_comma() {
        ...
        let pooled = vqa_core::pooling::pool(
            &[1.0, 2.0, 3.0],
            vqa_core::metric::HarmonicMean::Blocked("no"),
        )
        .unwrap();
        write_summary_csv(
            &path,
        ...
        assert!(row.contains(",,"), "the blocked harmonic mean is an empty cell");
    }
