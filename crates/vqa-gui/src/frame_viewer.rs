...
use vqa_core::frames::{FrameValue, step_from};
...
type Extraction = vqa_core::Result<vqa_run::ExtractedFrame>;
...
fn load(ui: &Ui, label: &'static str, path: &Path) -> Slot {
    let texture = vqa_run::read_png(path).ok().map(|image| {
...
fn placeholder(ui: &mut Ui, label: &str, width: f32, loading: bool) {
    ...
    ui.painter().text(
        ...
        text,
        egui::FontId::new(11.0, egui::FontFamily::Name(crate::fonts::MONO_FAMILY.into())),
...
fn controls(ui: &mut Ui, tokens: &Tokens, viewer: &mut FrameViewer) -> Ask {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        for gain in GAINS {
            ...
            let label = sans(
                format!("x{gain}"),
                11.0,
                if chosen { tokens.on_accent } else { tokens.text },
            );
            let button =
        ...
        ui.add_space(10.0);
        ui.checkbox(&mut viewer.actual_size, sans("1:1", 11.0, tokens.text_secondary))
    ...
    if !viewer.commands.is_empty() {
        ui.add_space(4.0);
        ui.collapsing(sans("the commands that made these", 11.0, tokens.text_muted), |ui| {
...
mod tests {
    use super::*;
    use vqa_core::frames::worst_frames;
    ...
    fn viewer_on(values: &[f32], direction: Direction, frame: u64) -> FrameViewer {
        ...
        viewer.open_at(
            ...
            metric,
            frame,
            worst_frames(values, 0, direction),
        );
        viewer
    }
