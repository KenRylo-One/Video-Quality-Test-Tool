...
pub fn parse_vmaf_csv(
    ...
    let column = column_name(metric).ok_or_else(|| {
        vqtt_core::CoreError::parse("vmaf csv", format!("{metric:?} has no libvmaf CSV column"))
    })?;

    let mut lines = reader.lines();
    let header = lines
        .next()
        .ok_or_else(|| vqtt_core::CoreError::parse("vmaf csv", "empty file"))?
        .map_err(|error| vqtt_core::CoreError::parse("vmaf csv", error.to_string()))?;
    let headers: Vec<&str> = header.split(',').collect();
    let column_index = headers
        .iter()
        .position(|name| *name == column)
        .ok_or_else(|| {
            vqtt_core::CoreError::parse("vmaf csv", format!("no {column} column in {header}"))
        })?;

    for line in lines {
        let line =
            line.map_err(|error| vqtt_core::CoreError::parse("vmaf csv", error.to_string()))?;
        ...
        let frame: u64 = fields
            ...
            .ok_or_else(|| vqtt_core::CoreError::parse("vmaf csv", format!("bad row: {line}")))?;
