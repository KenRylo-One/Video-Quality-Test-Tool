...
use vqtt_core::palette::{Theme, series_color};
use vqtt_run::ThemeChoice;
...
mod tests {
    ...
    fn the_plot_chrome_matches_the_tokens_of_the_same_theme() {
        for tokens in [DARK, LIGHT] {
            let chrome = vqtt_core::plot::Chrome::for_theme(tokens.theme);
            let same = |left: Color32, right: vqtt_core::plot::Rgba| {
                assert_eq!(
