...
pub fn parse_vship_json(
    ...
    reader
        ...
        .map_err(|error| vqa_core::CoreError::parse("vship json", error.to_string()))?;
    ...
    if metric == MetricId::Cvvdp {
        let last = rows
            .last()
            .ok_or_else(|| vqa_core::CoreError::parse("vship json", "empty json array"))?;
        let value = *last
            .get(column)
            .ok_or_else(|| vqa_core::CoreError::parse("vship json", "row has no column 0"))?;
    ...
    for (frame, row) in rows.iter().enumerate() {
        let value = *row.get(column).ok_or_else(|| {
            vqa_core::CoreError::parse("vship json", format!("row {frame} has no column {column}"))
        })?;
